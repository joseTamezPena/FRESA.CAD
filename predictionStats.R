predictionStats_survival <-  function(predictions, plotname="", noNegativeRisk=FALSE,...)
{
  ...
  Curves <- survFitBSWiMS <- survfit(Surv(times, status) ~ preds>=median(preds),newData)
  ...
  #Curves <- survFitBSWiMS <- survfit(Surv(predictions[,1], predictions[,2]) ~ predictions[,6]>=median(predictions[,6]))
  if(plotname!="")
  {
    if(noNegativeRisk)
    {
      graph<-ggsurvplot(Curves, data=newData, conf.int = TRUE, legend.labs=c("High Risk","Low Risk"),
                        palette = c("#ff0000", "#00bbff"),
                        ggtheme = theme_bw() + theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 30)),
                        title = plotname,
                        risk.table = TRUE,
                        tables.height = 0.2,
    ...
    {
      graph<-ggsurvplot(Curves, data=newData, conf.int = TRUE, legend.labs=c("Low Risk", "High Risk"),
  ...
  LogRank <- survdiff(Surv(predictions[,1], predictions[,2]) ~ predictions[,6]>=median(predictions[,6]))
...
concordance95ci <- function(datatest,nss=4000,followUp=FALSE)
{
  ...
  if (sz>2)
  {
    ...
    for (i in 1:nss)
    {
      ...
      if(followUp)
      {
        #	Quitar los NA, porque regreso NA.
