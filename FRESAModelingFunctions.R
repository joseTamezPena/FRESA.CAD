...
if (!requireNamespace("glmnet", quietly = TRUE)) {
   install.packages("glmnet", dependencies = TRUE)
		...
		if (sum(str_count(baseformula,"Surv")) > 0)
    {
      ...
      result <- list(fit = glmnet::cv.glmnet(as.matrix(data[,usedFeatures]),Surv(x,y),family = "cox"),s = s,formula = formula,usedFeatures=usedFeatures);
...
BESS <- function(formula = formula, data=NULL, ...)
{
  ...
  baseformula <- gsub(featuresOnSurvivalObject[[1]][1],"x",baseformula)
  baseformula <- gsub(featuresOnSurvivalObject[[1]][2],"y",baseformula)
  result <- list(fit=BeSS::bess(as.matrix(data[,usedFeatures]), Surv(x, y), s.min=1, family = "cox"),formula = formula,usedFeatures=usedFeatures);
  class(result) <- "bess"
