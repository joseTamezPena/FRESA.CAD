#'\code{CoxBenchmark}
#'
#'@param theData The data-frame for cross validation.
#'@param theOutcome  The name of the outcome. A Survival Object Surv(time,status)
#'@param reps  The number of times that the random cross-validation will be performed default=100
#'@param trainFraction  The fraction of the data used for training. default=0.5
#'@param referenceCV  A user supplied random cross-validation object to be benchmarked default=NULL
#'@param referenceName  The name of the reference classifier to be used in the reporting tables default="Reference"
#'@param referenceFilterName The name of the reference filter to be used in the reporting tables default=COX.BSWiMS"
#'
#'
#'\itemize{
#'\item errorciTable the matrix of the balanced error with the 95 CI
#'\item accciTable the matrix of the classification accuracy with the 95 CI
#'\item aucTable the matrix of the ROC AUC with the 95 CI
#'\item senTable the matrix of the sensitivity with the 95 CI
#'\item speTable the matrix of the specificity with the 95 CI
#'\item errorciTable_filter	the matrix of the balanced error with the 95 CI for filter methods
#'\item accciTable_filter the matrix of the classification accuracy with the 95 CI for filter methods
#'\item senciTable_filter the matrix of the classification sensitivity with the 95 CI for filter methods
#'\item speciTable_filter the matrix of the classification specificity with the 95 CI for filter methods
#'\item aucTable_filter	the matrix of the ROC AUC with the 95 CI for filter methods
#'\item CIRiskTable the matrix of the concordance index on Risk with the 95 CI 
#'\item CIFollowUpTable the matrix of the concordance index on Follow-up times with the 95 CI 
#'\item LogRankTable the matrix of the LogRank Test with the 95 CI 
#'\item CIRisksTable_filter the matrix of the concordance index on Risk with the 95 CI for the filter methods
#'\item CIFollowUpTable_filter the matrix of the concordance index on Follow-up times with the 95 CI for the filter methods 
#'\item LogRankTable_filter the matrix of the LogRank Test with the 95 CI for the filter methods
#'\item times The average CPU time used by the method
#'\item jaccard_filter The average Jaccard Index of the feature selection methods
#'\item TheCVEvaluations The output of the randomCV (randomCV) evaluations of the different methods
#'\item testPredictions A matrix with all the test predictions
#'\item featureSelectionFrequency The frequency of feature selection
#'\item cpuElapsedTimes	The mean elapsed times
#'}
